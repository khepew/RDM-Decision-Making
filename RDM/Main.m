...
%Exclude the not completed trials and those with 0% coherence
%accuracy_index      = find((result(:,2)~= 0) & (result(:,7) ~= 0));  
%accuracy            = sum(result(accuracy_index,5))/numel(accuracy_index);
completed_trials = result(result(:, 7) ~= 0, :);
accuracy = sum(completed_trials(:, 5)) / size(completed_trials, 1);
...
if mean_RT < 1 && accuracy >= 0.8
    feedback_msg = 'Keep it Up!';
elseif mean_RT < 1 && accuracy < 0.8
    feedback_msg = 'Try to be more accurate!';
elseif mean_RT >= 1 && accuracy >= 0.8
    feedback_msg = 'Try to be faster!';
else
    feedback_msg = 'Try to improve your speed and accuracy!';
end
